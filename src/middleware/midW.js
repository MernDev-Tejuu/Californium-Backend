  ...
  const authJWT = async (req,res,next)=>{
   try{ 
        ...
        for(i=0;i<requiredField.length;i++){
          ...
          const tokenCreate  = jwt.sign({_id : finder._id},"MERNSTACKDEVELOPMENT")
         
          const tokenVerification = jwt.verify(tokenCreate,"MERNSTACKDEVELOPMENT")  
  ...
  //Frontend About wala part =>{
    const authenticate = async(req,res,next)=>{
      try{
         ...
         const tokenVerification = jwt.verify(tokenFetch,"MERNSTACKDEVELOPMENT")
